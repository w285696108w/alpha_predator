...
AlphaPredator 支持多种可插拔数据源，可通过 `config/settings.py` 切换：

| 数据源 | 说明 | Token需求 | 推荐度 |
|--------|------|-----------|--------|
| `mock` | 纯模拟数据，无网络依赖 | 无 | 测试/CI |
| `akshare` | 新浪财经A股实时/历史数据 | 无 | ⭐⭐⭐ |
| `baostock` | Baostock免费A股历史数据 | 无 | ⭐⭐⭐⭐ |
| `tushare` | Tushare Pro全功能API | 需要(积分≥120) | ⭐⭐⭐⭐⭐ |

**切换方式**：

```python
# config/settings.py
CONFIG.data_source.source = 'baostock'
```

```bash
# 环境变量 (run_backtest.py / app.py)
$env:ALPHA_DATA_SOURCE = 'baostock'
...
mgr = DataManager(source='baostock')  # baostock/akshare/tushare/mock
```

**Tushare Token**： https://tushare.pro/register (积分需≥120)
**Baostock**：完全免费，无需注册 http://baostock.com
...
`app.py` 是 AlphaPredator 的可视化界面，基于 **Streamlit** + **Plotly**：
...
- **侧边栏参数**：开始/结束日期、初始资金、数据源（mock / akshare）、凯利系数、追踪止损幅度、信号阈值

- **一键回测**：对 5 只模拟标的（STOCK_A/B/C/D、INDEX_300）批量运行回测
...
- **核心指标卡**：总收益、年化收益、夏普比率、最大回撤、交易次数、胜率



**界面配色：** 深色主题（`#0E1117` 背景），K线绿（`#00C896`）/ 止损红（`#FF4B4B`）
...
# --mode   paper=模拟 / live=实盘（需 broker 接口）

# --symbols  股票代码列表（支持上交所/深交所）
...
**输出：** 每笔交易记录 + 持仓快照 + 日终报告（发送到终端或 webhook）
